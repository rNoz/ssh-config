...
== Modifications by rNoz

Just really tiny features over the project from _dbrady_.
...
- searching _Hosts_, _Keys_ or _Values_:
    search, find

- showing the help:
    -?, -h, --help, help

=== PKGBUILD for AUR/Arch packages

Located under +builder/arch+, providing the package builder for _AUR_ (https://aur.archlinux.org/) and _Pacman_ packages.
Recently uploaded to _AUR_ in https://aur.archlinux.org/packages/ssh-config/ (+packer -S ssh-config+, +yaourt ssh-config+).

== Original project

Original code (almost everything) from _dbrady_ (https://github.com/dbrady/ssh-config).

The +-?+ and +-h+ idea was suggested from the PR of _epower_ (https://github.com/epower/ssh-config), but because the merging process was/is slow, this repo is created.
